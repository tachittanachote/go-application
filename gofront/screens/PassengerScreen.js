  ...
  componentDidMount = async () => {
    ...
    requestGeolocationPermission().then((e) => {
      Geolocation.getCurrentPosition(
        (position) => {
          this.setState({
            coordinates: getDeltaCoordinates(
    ...
    axios
      ...
      .then((e) => {
        ...
        e.data.map((car, index) => {
          ...
          var location = this.state.carLocations.concat(
            Object.assign(car.coordinates, carInfo)
          );
    ...
    axios
      ...
      .then((e) => {
        ...
        this.setState(
          {
            travelInfo: e.data,
            marker: driver.coordinates.destination,
            mapViewDirection: (
              <MapViewDirections
          ...
          () => {
            setTimeout(() => {
              this.state.mapRef.fitToCoordinates(
                [
                  driver.coordinates.currentPosition,
                  driver.coordinates.destination,
                ],
                {
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        {this.state.coordinates === null ? (
          ...
          <View style={{ flex: 1 }}>
            ...
            >
              {this.state.marker && (
                <Marker coordinate={this.state.marker} identifier={"marker"}>
                  <Image
                    source={images.marker_icon}
                    style={{ ...MAPS.markerSize }}
                  ></Image>
                </Marker>
              )}
              ...
              {this.state.carLocations.length > 0 &&
                this.state.carLocations.map((car, index) => (
                  <Marker
                    key={index}
            ...
            {this.state.driverInformation === null ? (
                  ...
                  {this.state.drivers.length > 0 &&
                    this.state.drivers.map((driver, index) => (
                      <DriverInformation
                        key={index}
                        driver={driver}
                        onCallBack={(e) => this.getDriverInformation(e)}
                        availableSeat
                      ></DriverInformation>
                    ))}
                  {this.state.drivers.length === 0 && (
              ...
              >
                ...
                <PaymentOptions
                  balance={this.context.user.wallet_balance}
                  estPrice={this.state.travelInfo.price}
                  onPaymentOptionCallback={(e) => {
                    this.onPaymentOptionCallback(e)
                  }}
                ></PaymentOptions>
                ...
                >
                  ระยะทางโดยประมาณ {this.state.travelInfo.distance} km
                </Text>
                <Text
                  style={{
                    color: COLORS.lightGray2,
                ...
                >
                  ค่าโดยสารโดยประมาณ {this.state.travelInfo.price} บาท
                </Text>
                ...
                <TouchableWithoutFeedback
                  onPress={() => this.handleCarCall(this.state.selectedCarId)}
                >
                  <View
                    style={{
                      borderRadius: SIZES.radius - 5,
