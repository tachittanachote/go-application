...
exports.addWalletTransaction = (wallet) => {
    return new Promise((resolve, reject) => {
        db.query(`INSERT INTO wallet_transactions (wallet_transaction_id, user_id, amount, type) VALUES (?, ?, ?, ?)`, 
            [
                wallet.id, 
                wallet.user_id, 
                wallet.amount,
...
exports.getPendingWalletTransactionByUserId = (id) => {
    return new Promise((resolve, reject) => {
        db.query(`SELECT * FROM wallet_transactions WHERE user_id = ?`, [id], (err, result) => {
            if (err) reject(err);
            resolve(result);
        });
    });
